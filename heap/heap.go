...
// Dump out all `node.key`s in a slice
...
// Dump out all `node.score`s in a slice
...
// Return top n `node.key`s and associated `node.score`s in two sorted slices
